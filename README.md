...
- **静音播放**：浏览器自动静音，避免干扰用户,你还可以在前台使用其他真实输入/输出设备打游戏或听歌而不受影响。
- **多任务支持**：现支持更多种类的任务。
- **多账户支持**：在不改变原有功能的基础上，可以同时运行多个不同账户的任务，只需要在user.json中配置不同的用户信息即可；可选择多种运行模式，如GPU运行，CPU运行，CPU/GPU混合运行等，一切详细配置说明参照main.py开头的注释。
